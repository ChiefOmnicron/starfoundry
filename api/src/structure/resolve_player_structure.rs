use sqlx::PgPool;
use starfoundry_libs_structures::{ResolvedStructure, StructureService};
...
pub async fn resolve_player_structure(
    ...
    match StructureService::resolve_player_structure(
        ...
        Err(starfoundry_libs_structures::Error::InvalidStructureId(_)) => {
            Err(ReplyError::BadRequest.into())
        }
        Err(starfoundry_libs_structures::Error::FetchPlayerStructureFromEve(e, _)) => {
            tracing::error!("{}", e);
