...
import { Route } from './$industryHubId.route';
...
describe('Structure Group Header', () => {
    it('makes sure that everything is loaded and correctly shown', () => {
        cy.intercept(
            {
                method: 'GET',
                url: '/api/industry-hubs/e52744df-a140-4e72-b24c-18f534d0ae94',
            },
            (req) => {
                req.reply({
                    body: {
        ...
        ).as('dfetchIndustryHub');

        cy.mount(componentMount());

        cy.get('[data-cy="loading"]').should('be.visible');
        cy.wait('@dfetchIndustryHub');
