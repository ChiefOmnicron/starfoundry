...
export class ItemService {
    ...
    public static async blueprints(): Promise<IItem[]> {
        return (
            await axios.get('/api/v1/items/blueprints')
        ).data;
    }
    ...
    public static async resolve_id(tid: TypeId): Promise<IItem> {
        if (this.cache[tid]) {
            ...
            let entry: IItem = (
                await axios.get(`/api/v1/items/resolve/${tid}`)
            ).data;
    ...
    // TODO: add buildable
    public static async resolve_names_bulk(names: string[]): Promise<IBulkItem[]> {
        return (
    ...
    public static async parse<T>(
        ...
        return (await axios.post('/api/v1/items/parse', content, {
            params: {
                blueprint,
            },
            headers: {
    ...
    public static async resolve_names_from_rows(rows: string): Promise<IParsedRow[]> {
