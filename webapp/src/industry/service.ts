...
export class Service {
  ...
  public static async industry_index(
    ...
    return (await axios.get(`${INDUSTRY_INDEX_PATH}?system_id=${system_id}`)).data;
  }

  public static async weighted_average(
    type_ids: { [key: number]: number }
  ): Promise<any> {
    return (await axios.post(`${INDUSTRY_WEIGHTED_AVERAGE}`, type_ids)).data;
  }
}
