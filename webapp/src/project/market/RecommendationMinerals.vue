<template>
  ...
  class ProjectMarketRecommendation extends Vue {
    ...
    public recommendations: {
      [key: string]:
      { export: string, import: string, source: string, price_total: number }
    } = {};
    ...
    public async bought(source: string) {
      ...
      this.busy = true;
      await Service.update_market_minerals(<any>this.$route.params.projectId, parsed);
      await this.load();
      this.busy = false;
    }
    ...
    private async load() {
      ...
      let prices = await Service.fetch_market_recommendation_minerals(<any>this.$route.params.projectId);
