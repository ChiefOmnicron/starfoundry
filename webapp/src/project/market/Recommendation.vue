        ...
        >
            <template #title>
                <div>
                    ...
                    <small>
                        Last fetch:
                        <format-number date :utc="false" :value="entry.last_fetch" />
                    </small>
                </div>
            </template>
            ...
            <template #action>
                ...
                <n-button :disabled="busy" @click="bought(<string>entry.source)" type="info">
                    Bought
                </n-button>
            </template>
            ...
            <n-grid :cols="22">
                <n-grid-item span="10">
                    <n-input :value="entry.export" type="textarea" disabled></n-input>
                </n-grid-item>
                <n-grid-item span="2">
                ...
                <n-grid-item span="10">
                    <n-input v-model:value="entry.import" type="textarea"></n-input>
                </n-grid-item>
            </n-grid>
...
import { NButton, NEmpty, NGrid, NGridItem, NIcon, NInput, NSpace } from 'naive-ui';
...
class ProjectMarketRecommendation extends Vue {
    ...
    public async bought(source: string) {
        let item_names = (this.recommendations.find(x => x.source === source) || { import: ''}).import
            .split('\n')
        ...
        let parsed: IUpdateMarketEntry[] = (this.recommendations.find(x => x.source === source) || { import: ''}).import
            .split('\n')
            .filter((x: string) => !x.startsWith('Total'))
            .filter((x: string) => x !== '')
            .map(x => {
        ...
        await Service.update_market_general(<any>this.$route.params.projectId, parsed);
        await this.load();
    }
    ...
    private async load() {
        ...
        let prices = await Service.fetch_market_recommendation_general(<any>this.$route.params.projectId);
        ...
        for (let price of prices) {
            if (lastFetched.has(price.structure_id)) {
                ...
                lastFetched.set(price.structure_id, new Date(lastFetch).getTime());
            }
        }

        for (let price of prices) {
            let recommendation: IRecommendedMarket | undefined = recommendations.find(x => x.source === price.source);
            if (!recommendation) {
                recommendation = {
                    export: '',
                    import: '',
        ...
        recommendations = recommendations.sort((a: IRecommendedMarket, b: IRecommendedMarket) => b.source > a.source ? -1 : 1);
