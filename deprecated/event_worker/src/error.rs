use starfoundry_lib_types::{CharacterId, CorporationId, RegionId, StationId};
use thiserror::Error;
use uuid::Uuid;
...
pub enum Error {
    ...
    ApiError(starfoundry_lib_eve_api::Error),
    ...
    // indy tasks
    #[error("error fetching character ids, '{0}'")]
    FetchCharacterIds(sqlx::Error),
    #[error("error fetching character ids from queue, '{0}'")]
    ...
    #[error("error while inserting industry index, '{0}'")]
    InsertIndustryIndex(sqlx::Error),
    #[error("error while inserting industry jobs, '{0}'")]
    ...
    // stock
    #[error("error while fetching notifications, error: '{0}'")]
    FetchStockBlueprintNotifications(sqlx::Error, Uuid),

    #[error("sde error {0}")]
    SdeError(starfoundry_lib_eve_sde_parser::Error),
    #[error("eve api error {0}")]
    EveApiError(starfoundry_lib_eve_api::Error),
    #[error("notification error: '{0}'")]
    NotificationError(starfoundry_lib_notification::Error),

    // stock
    #[error("error while fetching blueprint stocks, error: '{0}'")]
