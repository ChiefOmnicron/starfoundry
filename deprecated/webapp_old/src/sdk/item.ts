...
export class ItemService {
    public static async parse<T>(
        ...
        return (
            await axios.post(`${ITEM_PATH}/parse`, content, {
                params: {
                    blueprint,
                },
                headers: {
    ...
    public static async get(type_id: TypeId): Promise<IItem> {
        ...
        const result = (await axios.get(`${ITEM_PATH}/resolve/ids/${type_id}`))
            .data;
    ...
    public static async get_bulk(type_ids: TypeId[]): Promise<IItem[]> {
        let cached: IItem[] = [];
        ...
        if (remaining.length > 0) {
            const result = (
                await axios.post(`${ITEM_PATH}/resolve/ids`, remaining)
            ).data;

            result.forEach((result: IItem) => {
