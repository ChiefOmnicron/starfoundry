...
class ProjectMarketRecommendation extends Vue {
    ...
    public async bought(source: string) {
        ...
        this.busy = true;
        await Service.update_market_gas(
    ...
    private async load() {
        ...
        let prices = await Service.fetch_market_recommendation_gas(
            <any>this.$route.params.projectId,
        );
        ...
        for (let price of prices) {
            ...
            this.recommendations[price.source].export +=
                `${price.item_name}\t${price.quantity}\n`;
            this.recommendations[price.source].import +=
                `${price.item_name}\t${price.quantity}\t${price.price}\t${price.quantity * price.price}\n`;
            this.recommendations[price.source].price_total +=
                price.quantity * price.price || 0;
            this.recommendations[price.source].volume_total +=
                price.quantity * price.volume || 0;
        }
    }
}
