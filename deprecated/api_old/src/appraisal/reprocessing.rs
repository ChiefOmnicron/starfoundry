use sqlx::PgPool;
use starfoundry_lib_appraisal::internal::{Appraisal, ReprocessingOptions};
...
#[utoipa::path(
    ...
    path = "/appraisals/{code}/reprocessing",
    tag = "appraisal",
    params(
        ("code" = String, Path),
    ),
...
pub async fn reprocessing(
    ...
    match starfoundry_lib_appraisal::internal::reprocessing(
        &pool,
        code,
        options,
    ).await {
