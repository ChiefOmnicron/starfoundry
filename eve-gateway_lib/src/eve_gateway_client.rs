...
pub const ENV_EVE_GATEWAY_API_URL: &str = "STARFOUNDRY_EVE_GATEWAY_API_URL";
...
impl GatewayClient {
    ...
    fn api_url(&self) -> Result<Url> {
        let env = if let Ok(x) = std::env::var(ENV_EVE_GATEWAY_API_URL) {
            x
        } else {
            return Err(EveGatewayClientError::EnvNotSet(ENV_EVE_GATEWAY_API_URL).into());
        };
