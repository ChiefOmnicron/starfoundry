...
function RouteComponent() {
    ...
    const optionData = () => {
        ...
        for (const option of product.additional_options) {
            let entry = products.find(x => x.uuid === option.reference_id);
    ...
    const contentAddition = () => {
        ...
        let selectedProduct: Product | undefined = products.find(x => x.uuid === selectedAddition);
        if (!selectedProduct) {
            if (additionalOptionPrice !== 0) {
                setAdditionalOptionPrice(0);
            }
    ...
    return <>
        ...
        >
            <Stack style={{ width: '100%' }}>
                ...
                <Title order={4}>3 to 4 weeks</Title>
                <Text>This is only an estimated, please give us time at the start to improve the processes in the background</Text>
