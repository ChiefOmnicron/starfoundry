...
pub fn parse(
    directory: &str,
) -> Result<Vec<AsteroidBelt>, Error> {
    tracing::info!("Parsing mapAsteroidBelts.yaml");
    ...
    let parsed: HashMap<OrbitId, AsteroidBeltWrapper> = serde_yaml::from_reader(file)
        .map(|x| {
            tracing::info!(
                "Finished parsing mapAsteroidBelts.yaml, task took {:.2}s",
    ...
    let parsed = parsed
        .into_iter()
        .map(|(orbit_id, wrapper)| AsteroidBelt {
            asteroid_belt_id:   orbit_id,
...
pub struct AsteroidBeltWrapper {
