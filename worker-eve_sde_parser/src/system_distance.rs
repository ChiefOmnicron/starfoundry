...
pub async fn run(
    ...
    for system_a in systems.iter() {
        for system_b in systems.iter() {
            ...
            if system_a.system_id == SystemId(30004807) && system_b.system_id == SystemId(30004831) {
                dbg!(system_a.position.x);
                dbg!(system_a.position.y);
                dbg!(system_a.position.z);

                dbg!(system_b.position.x);
                dbg!(system_b.position.y);
                dbg!(system_b.position.z);
            }
